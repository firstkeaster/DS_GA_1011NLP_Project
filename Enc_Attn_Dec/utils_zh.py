...
class Lang:
    def __init__(self, name):
        ...
        self.word2count = {"<pad>": 65535, "<unk>": 65534, "SOS": 65533, "EOS": 65532}
    ...
    def addWord(self, word):
        if word not in self.word2index:
            ...
            self.word2count[word] += 1
    def topK(self, K=50000):
        topKWords = sorted([word for word in self.word2index.keys()], key=lambda x:-word2count[x])[:K]
        for word in list(self.word2count.keys()):
            if word not in topKWords:
                self.index2word.pop(self.word2index[word])
...
def ConvertSentence2numpy(input_lang, output_lang, pairs, UNK_token, EOS_token, MAX_LEN = 100):
    input_sentences = []
    output_sentences = []
    if MAX_LEN == None:
