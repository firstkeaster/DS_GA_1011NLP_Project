...
def dataset_loss(encoder, decoder, dataloader, criterion = nn.NLLLoss().cuda()):
    ...
    with torch.no_grad():
        for i, (input_sentences, target_sentences) in enumerate(dataloader):
            ...
            # encoder_outputs = torch.zeros(input_sentences.size()[0], MAX_val, 2*HIDDEN_SIZE)
...
def evaluate(loader, encoder, decoder, output_lang, MAX_LENGTH = 100):
    ...
    with torch.no_grad():
        for i, (input_sentence, output_sentence) in enumerate(loader):
            ...
            #encoder_outputs, encoder_hidden = encoder(input_sentence.to(device), None)
            # encoder_outputs = torch.zeros(input_sentence.size()[0], MAX_val, 2*HIDDEN_SIZE)
            # a, encoder_hidden = encoder(input_sentence.to(device), None)
            # encoder_outputs[:, :input_sentence.size()[1], :] = a
            encoder_outputs = encoder(input_sentence.to(device), None)
            encoder_hidden = encoder_outputs[:,-2:,:].transpose(0,1).contiguous()
            encoder_hidden = encoder_hidden.to(device)
            encoder_outputs = encoder_outputs.to(device)
