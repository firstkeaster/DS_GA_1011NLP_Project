...
def filter_target(input_lang, pairs, MAX_LEN, UNK_token):
    target_filter = []
    for i in range(len(pairs)):
        s1 = indexesFromSentence(input_lang, pairs[i][0], UNK_token)
        if len(s1)<MAX_LEN:
            target_filter.append(pairs[i][1])
...
class TransDataset(Dataset):
    ...
    def __init__(self, input_sentences, target_sentences, sort = False):
        """
        ...
        if sort == True:
            ...
            for i in range(len(input_sentences)):
                input_length.append(input_sentences[i].shape[0])
            ind_inc_order = np.argsort(np.array(input_length))
            a, b = [], []
            for i in range(len(ind_inc_order)):
                k = ind_inc_order[i]
                a.append(input_sentences[k])
                b.append(target_sentences[k])
            self.input_sentences, self.target_sentences = a, b
        else:
